...
test("dashboard assets remain part of the shipped source tree", async () => {
  ...
  assert.doesNotMatch(script, /text-shimmer-glyph|SHIMMER_STEP_MS|glyph\.animate/);
  assert.match(styles, /\.text-shimmer \{[\s\S]*width: fit-content;[\s\S]*justify-self: start;/);
  assert.match(styles, /\.text-shimmer \{[\s\S]*linear-gradient\(100deg, var\(--muted\) 48%, var\(--text\) 50%, var\(--muted\) 52%\);/);
  assert.match(styles, /\.text-shimmer \{[\s\S]*animation: text-shimmer-sweep 6\.4s linear infinite;/);
  assert.match(styles, /@keyframes text-shimmer-sweep \{[\s\S]*72%, 100% \{ background-position: -100% 0; \}/);
