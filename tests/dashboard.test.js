...
import { priorCompatibleVersion } from "../src/dashboard-manager.js";
import {
  clearNotifications,
  dismissNotification,
  filterTasks,
...
test("dashboard handoff uses complete SemVer precedence", () => {
  ...
  assert.equal(priorCompatibleVersion("7.23.0+old", "7.23.0+new"), false);
  assert.equal(priorCompatibleVersion("7.23.1", "7.23.0"), false);
  assert.equal(priorCompatibleVersion("7.23.0-01", "7.23.0-2"), false);
  assert.equal(
    priorCompatibleVersion("9007199254740993.0.0", "9007199254740994.0.0"),
    true,
  );
});
...
test("same-version reuse restores the dashboard after the original owner closes", async () => {
  const { workspace } = await fixture();
  const port = await unusedPort();
  const original = createDashboardManager({ workspace, port });
  const replacement = createDashboardManager({
    workspace,
    port,
    reuseMonitorIntervalMs: 100,
  });
  try {
    assert.equal((await original.ensure()).action, "started");
    assert.equal((await replacement.ensure()).action, "reused");
    assert.equal(replacement.owned, false);
    await original.close();
    await waitFor(() => replacement.owned);
...
test("same-version reuse recovers when the binder cannot publish ownership", async () => {
  const { workspace } = await fixture();
  const port = await unusedPort();
  ...
  const binder = createDashboardManager({
    workspace,
    port,
  ...
  const peer = createDashboardManager({ workspace, port, reuseMonitorIntervalMs: 20 });
  try {
    const binderEnsure = binder.ensure();
    const binderFailure = assert.rejects(binderEnsure, /simulated owner publication failure/);
    await ready;
    assert.equal((await peer.ensure()).action, "reused");
    releasePublication();
    await binderFailure;
    await waitFor(() => peer.owned);
    assert.equal((await readDashboardIdentity({ port })).workspace, await realpath(workspace));
  } finally {
    releasePublication();
    await Promise.allSettled([binder.close(), peer.close()]);
  }
});

test("same-version reuse keeps monitoring across a temporary unknown occupant", async () => {
  const { workspace } = await fixture();
  const port = await unusedPort();
  const original = createDashboardManager({ workspace, port });
  const replacement = createDashboardManager({
  ...
  const unknown = http.createServer((_request, response) => {
    ...
    response.end(JSON.stringify({ service: "not-taskchef" }));
  });
  try {
    ...
    assert.equal(replacement.owned, false);
    await new Promise((resolve, reject) => unknown.close((error) => error ? reject(error) : resolve()));
    await waitFor(() => replacement.owned);
    assert.equal((await readDashboardIdentity({ port })).workspace, await realpath(workspace));
  } finally {
    if (unknown.listening) {
...
test("authenticated handoff supports the IPv6 loopback host", async () => {
  const { workspace } = await fixture();
  ...
  const newManager = createDashboardManager({
    workspace,
    host: "::1",
    port,
  ...
  try {
    ...
    assert.equal((await readDashboardIdentity({ host: "::1", port })).taskchefVersion, "7.23.0");
  } finally {
    await Promise.allSettled([oldManager.close(), newManager.close()]);
  }
});
...
test("dashboard manager performs authenticated handoff from an older compatible MCP version", async () => {
  const { workspace } = await fixture();
  const port = await unusedPort();
  const oldManager = createDashboardManager({ workspace, port, taskchefVersion: "7.22.1" });
  const newManager = createDashboardManager({ workspace, port, taskchefVersion: "7.23.0" });
  try {
    assert.equal((await oldManager.ensure()).action, "started");
    const ownerInfo = await lstat(path.join(workspace, DASHBOARD_OWNER_FILE));
    assert.equal(ownerInfo.mode & 0o777, 0o600);
    assert.equal((await newManager.ensure()).action, "started");
    assert.equal(oldManager.owned, false);
    await assert.rejects(oldManager.ensure(), /shutting down/);
...
test("concurrent authenticated handoff elects one new owner and reuses it idempotently", async () => {
  const { workspace } = await fixture();
  const port = await unusedPort();
  const oldManager = createDashboardManager({ workspace, port, taskchefVersion: "7.22.1" });
  const first = createDashboardManager({ workspace, port, taskchefVersion: "7.23.0" });
  const second = createDashboardManager({ workspace, port, taskchefVersion: "7.23.0" });
  try {
    ...
    assert.equal((await readDashboardIdentity({ port })).taskchefVersion, "7.23.0");
    assert.equal((await readDashboardOwner(await realpath(workspace))).taskchefVersion, "7.23.0");
  } finally {
    await Promise.allSettled([oldManager.close(), first.close(), second.close()]);
  }
});

test("authenticated handoff re-verifies a same-version peer that recovers during shutdown", async () => {
  const { workspace } = await fixture();
  const port = await unusedPort();
  let publishPeer;
  let peerBound;
  const publicationGate = new Promise((resolve) => { publishPeer = resolve; });
  const bound = new Promise((resolve) => { peerBound = resolve; });
  const keepAlive = setInterval(() => {}, 1_000);
  const original = createDashboardManager({ workspace, port, taskchefVersion: "7.22.1" });
  const peer = createDashboardManager({
    workspace,
    port,
    taskchefVersion: "7.22.1",
    reuseMonitorIntervalMs: 10,
    createServer: async (options) => {
      const server = await createDashboardServer(options);
      peerBound();
      return server;
    ...
    writeOwner: async (...args) => {
      await publicationGate;
      await writeDashboardOwner(...args);
    },
  });
  ...
  const replacement = createDashboardManager({
    ...
    readIdentity: async (options) => {
      try {
        const identity = await readDashboardIdentity(options);
        if (!synchronized && !original.owned && identity.taskchefVersion === "7.22.1") {
          synchronized = true;
          await bound;
        }
        return identity;
      } catch (error) {
        if (!synchronized && !original.owned
            && ["ECONNREFUSED", "ECONNRESET"].includes(error?.code)) {
          synchronized = true;
          await bound;
          return readDashboardIdentity(options);
        }
        throw error;
      }
    },
  ...
  try {
    await original.ensure();
    assert.equal((await peer.ensure()).action, "reused");
    const replacementEnsure = replacement.ensure();
    await bound;
    await new Promise((resolve) => setTimeout(resolve, 50));
    publishPeer();
    assert.equal((await replacementEnsure).action, "started");
    assert.equal(peer.owned, false);
    assert.equal((await readDashboardIdentity({ port })).taskchefVersion, "7.23.0");
  } finally {
    clearInterval(keepAlive);
    publishPeer();
    await Promise.allSettled([original.close(), peer.close(), replacement.close()]);
  }
});

test("a concurrent activation waits for older owner metadata publication before handoff", async () => {
  const { workspace } = await fixture();
  const port = await unusedPort();
  let publishOwner;
  let serverReady;
  const publicationGate = new Promise((resolve) => { publishOwner = resolve; });
  const ready = new Promise((resolve) => { serverReady = resolve; });
  const oldManager = createDashboardManager({
    workspace,
    port,
  ...
  const newManager = createDashboardManager({ workspace, port, taskchefVersion: "7.23.0" });
  try {
    const oldEnsure = oldManager.ensure();
    await ready;
    const newEnsure = newManager.ensure();
    await new Promise((resolve) => setTimeout(resolve, 50));
    publishOwner();
    assert.equal((await oldEnsure).action, "started");
    assert.equal((await newEnsure).action, "started");
    assert.equal((await readDashboardIdentity({ port })).taskchefVersion, "7.23.0");
  } finally {
    publishOwner();
    await Promise.allSettled([oldManager.close(), newManager.close()]);
...
test("authenticated handoff force-closes a slow in-flight dashboard request", async () => {
  const { workspace } = await fixture();
  const port = await unusedPort();
  const oldManager = createDashboardManager({ workspace, port, taskchefVersion: "7.22.1" });
  const newManager = createDashboardManager({ workspace, port, taskchefVersion: "7.23.0" });
  ...
  try {
    ...
    slowRequest = http.request({
      host: "127.0.0.1",
      port,
      path: `/api/tasks/${FIRST_ID}/manual-transition`,
      method: "POST",
      headers: {
        Host: `127.0.0.1:${port}`,
    ...
    slowRequest.write("{");
    await new Promise((resolve) => slowRequest.once("socket", (socket) => {
...
test("dashboard control rejects missing, bad, and replayed credentials", async () => {
  const { workspace } = await fixture();
  ...
  try {
    const missingChallenge = await fetch(`${server.origin}${DASHBOARD_CONTROL_CHALLENGE_PATH}`);
    assert.equal(missingChallenge.status, 400);
    const missing = await fetch(`${server.origin}${DASHBOARD_CONTROL_SHUTDOWN_PATH}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
    });
    assert.equal(missing.status, 403);
    const bad = await fetch(`${server.origin}${DASHBOARD_CONTROL_SHUTDOWN_PATH}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ nonce: "b".repeat(48), proof: "c".repeat(64) }),
    });
    assert.equal(bad.status, 403);
    const nonce = createDashboardControlNonce();
    const authenticatedBody = JSON.stringify({
      nonce,
      proof: dashboardControlProof(secret, "shutdown", nonce),
    });
    const accepted = await fetch(`${server.origin}${DASHBOARD_CONTROL_SHUTDOWN_PATH}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: authenticatedBody,
    });
    assert.equal(accepted.status, 202);
    const replayed = await fetch(`${server.origin}${DASHBOARD_CONTROL_SHUTDOWN_PATH}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: authenticatedBody,
    });
...
test("dashboard manager never sends shutdown to a spoofed older listener", async () => {
  const { workspace } = await fixture();
  ...
  await writeDashboardOwner(canonicalWorkspace, dashboardOwnerMetadata({
    ...
    host: "127.0.0.1",
    port,
    taskchefVersion: identity.taskchefVersion,
  ...
  const listener = http.createServer((request, response) => {
    ...
    if (request.url?.startsWith(DASHBOARD_CONTROL_CHALLENGE_PATH)) {
      response.writeHead(200, { "Content-Type": "application/json" });
      response.end(JSON.stringify({ schemaVersion: 1, nonce: "wrong", proof: "e".repeat(64) }));
      return;
    }
  ...
  const manager = createDashboardManager({ workspace, port, taskchefVersion: "7.23.0" });
  try {
    await assert.rejects(manager.ensure(), /did not prove control/);
...
test("dashboard manager leaves an uncooperative legacy MCP listener untouched", async () => {
  const { workspace } = await fixture();
  const port = await unusedPort();
  const legacy = await createDashboardServer({
    workspace,
    port,
    launcher: "mcp",
    taskchefVersion: "7.22.0",
  });
  const manager = createDashboardManager({ workspace, port, taskchefVersion: "7.23.0" });
  try {
    await assert.rejects(
      manager.ensure(),
      /verified older TaskChef 7\.22\.0 listener without usable authenticated handoff metadata/,
    );
    assert.equal(legacy.monitor.closed, undefined);
    assert.equal((await readDashboardIdentity({ port })).taskchefVersion, "7.22.0");
  } finally {
    await manager.close();
    await legacy.close();
  }
...
test("dashboard ownership rejects unsafe records without blocking listener close", async () => {
  const { workspace } = await fixture();
  const port = await unusedPort();
  const manager = createDashboardManager({ workspace, port });
  await manager.ensure();
  ...
  const realOwnerPath = `${ownerPath}.real`;
  await rename(ownerPath, realOwnerPath);
  await symlink(realOwnerPath, ownerPath);
  await assert.rejects(readDashboardOwner(await realpath(workspace)), (error) => error?.code === "ELOOP");
});

test("durable dashboard ownership is retained after close and atomically replaced by the next owner", async () => {
  const { workspace } = await fixture();
  const port = await unusedPort();
  const first = createDashboardManager({ workspace, port });
  await first.ensure();
  const firstOwner = await readDashboardOwner(await realpath(workspace));
  await first.close();
  assert.deepEqual(await readDashboardOwner(await realpath(workspace)), firstOwner);
  const second = createDashboardManager({ workspace, port });
  await second.ensure();
  const secondOwner = await readDashboardOwner(await realpath(workspace));
  assert.notEqual(secondOwner.secret, firstOwner.secret);
  await second.close();
  assert.deepEqual(await readDashboardOwner(await realpath(workspace)), secondOwner);
});
