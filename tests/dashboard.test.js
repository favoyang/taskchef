...
test("dashboard assets remain part of the shipped source tree", async () => {
  ...
  assert.match(html, /id="notifications"[^>]+popover="manual"/);
  assert.match(html, /id="notification-host"[\s\S]*id="notifications"/);
  ...
  assert.match(script, /elements\.notifications\.showPopover/);
  assert.match(script, /const notificationHost = elements\.dialog\.open \? elements\.dialog : elements\.notificationHost/);
  assert.match(script, /if \(movingLayer\) notificationHost\.append\(elements\.notifications\)/);
  assert.match(script, /elements\.dialog\.showModal\(\);\s+renderNotifications\(\);/);
  assert.match(script, /state\.detailNavigation = \{[\s\S]*focusPending: true,[\s\S]*highlightTurnRef/);
  assert.match(script, /openDialog\(updated, \{ preserveNavigation: true \}\)/);
  assert.match(script, /const focusedElement = elements\.dialogResults\.contains/);
  assert.match(script, /while \(focusedTurn[\s\S]*focusedTurn = focusedTurn\.parentNode/);
  assert.match(script, /descendantWithFocusKey\([\s\S]*focusedDescendantKey/);
  assert.match(script, /replacedFocusedElement\.focus/);
  assert.match(script, /const preservedUsage = state\.selectedTask\?\.id === task\.id/);
  assert.match(script, /usage: task\.usage \?\? preservedUsage \?\? null/);
  assert.match(script, /requestGeneration === detailRequestGeneration[\s\S]*showMessage\("Task activity timeline is temporarily unavailable\."\)/);
  assert.match(dashboardLifecycle, /!\[Pending token usage in the task detail view\]\(images\/dashboard-token-pending\.jpg\)/);
  assert.match(script, /highlightTurnRef: notification\.turnRef \?\? notification\.turnId/);
  ...
  assert.match(script, /const SHIMMER_STEP_MS = 420;/);
  assert.match(script, /const SHIMMER_PAUSE_MS = 1_800;/);
  assert.match(script, /const SHIMMER_PULSE_MS = 420;/);
  assert.match(script, /const SHIMMER_ANIMATION_ID = "taskchef-text-shimmer";/);
  assert.match(script, /renderTextShimmer\(result, latest\.resultSummary\)/);
  assert.match(script, /renderTextShimmer\(result, presentation\.summary\)/);
  assert.match(script, /const pulseEnd = SHIMMER_PULSE_MS \/ record\.cycleMs;/);
  assert.match(script, /delay: shimmerIndex \* SHIMMER_STEP_MS,/);
  assert.match(script, /duration: record\.cycleMs,/);
  assert.match(script, /shimmerMotionQuery\?\.addEventListener\?\.\("change"/);
  assert.match(script, /for \(const record of shimmerRecords\) animateShimmerRecord\(record, matches\);/);
  assert.match(script, /shimmerRecords\.delete\(record\);/);
  assert.match(script, /accessibleText\.className = "visually-hidden";/);
  assert.match(script, /animatedText\.setAttribute\("aria-hidden", "true"\);/);
  assert.match(script, /cancelTextShimmers\(elements\.taskList\);/);
  assert.match(script, /cancelTextShimmers\(elements\.dialogResults\);/);
  assert.match(script, /cancelTextShimmers\(elements\.dialogUsage\);/);
  assert.match(styles, /@media \(prefers-reduced-motion: reduce\) \{\s*\.text-shimmer-glyph \{ color: currentcolor;/);
  assert.doesNotMatch(styles, /background-position|background-clip/);
