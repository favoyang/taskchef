...
const definitions = [
  ...
  },
  {
    ...
    ageMs: 12 * 60 * 60 * 1_000,
    durationMs: (18 * 60 + 32) * 1_000,
    followUp: {
      turnRef: "01a03275-d535-7043-ab4a-513a1ad6ae1e",
      idleMs: 5 * 60 * 60 * 1_000,
      durationMs: (2 * 60 + 14) * 60 * 1_000,
    },
    terminal: "completed",
  },
  {
  ...
  },
  {
    ...
    ageMs: 90 * 60 * 1_000,
    durationMs: 11 * 60 * 1_000,
    terminal: "completed",
  },
  {
    id: "66666666-6666-4666-8666-666666666666",
    threadId: "019ffb69-57a6-7801-8b7a-8ff4c32a3991",
    turnRef: "01a03275-d536-7043-ab4a-513a1ad6ae1e",
    title: "Inspect unavailable reported timing",
    request: "Verify the quiet unavailable state for a historical timestamp boundary.",
    ageMs: 60 * 60 * 1_000,
    durationMs: 9 * 60 * 1_000,
    terminal: "completed",
  },
];
...
for (const definition of definitions) {
  ...
  if (definition.terminal) {
    ...
    }, { now: timestamp(completedAt) });
    if (definition.followUp) {
      const followUpStartedAt = completedAt + definition.followUp.idleMs;
      await reportTaskState(workspace, {
        taskId: definition.id,
        threadId: definition.threadId,
        turnRef: definition.followUp.turnRef,
        turnId: null,
        status: "working",
        requestSummary: "Verify a second result after an idle follow-up gap.",
      }, { now: timestamp(followUpStartedAt) });
      await reportTaskState(workspace, {
        taskId: definition.id,
        threadId: definition.threadId,
        turnRef: definition.followUp.turnRef,
        turnId: null,
        status: definition.terminal,
        summary: "The second safe fixture turn completed successfully.",
      }, { now: timestamp(followUpStartedAt + definition.followUp.durationMs) });
      definition.turnRef = definition.followUp.turnRef;
    }
  }
}
